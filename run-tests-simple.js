...
// 加载MarkdownRenderer类
const MarkdownRenderer = require('./js/markdown.js');
...
async function testCreateFolderSecurity() {
    ...
    try {
        ...
        const rootResponse = await fetch('http://localhost:8000/api/folder/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function testMarkdownRendering() {
    ...
    try {
        ...
        if (typeof MarkdownRenderer !== 'undefined') {
            ...
            const html = renderer.render(testMarkdown);
            console.log('Markdown渲染测试:', html ? '成功' : '失败');
